...
(function () {
	...
	SznAutocompleteLink.prototype._setValue = function (value) {
		...
		if (value) {
                        console.log(this);
                        var previousValues = '';
                        if(!angular.isUndefined(this._$scope[this._$attrs["ngModel"]]))
                            var previousValues = this._$scope[this._$attrs["ngModel"]].split(";");

			var oldValues = "";
			for(var index in previousValues){
				
				if((previousValues[index] == value)){
					value = "";
				}

				if(this._validateEmail(previousValues[index]))
					oldValues += previousValues[index] + ";";
			}

			this._$scope[this._$attrs["ngModel"]] = oldValues + value;

			this._$scope.$digest();
		}
	};

	SznAutocompleteLink.prototype._validateEmail = function(email)
	{
		var re = /\S+@\S+\.\S+/;
